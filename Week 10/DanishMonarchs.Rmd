...
#command+option+i for at få det der ```{r} man kan skrive flere commands i den
```{r} 
#Step 1 install the tidyverse
library(tidyverse)
...
-  `read_csv2()`
```{r}
# Step 1 FILL IN THE CODE BELOW and review the outputs and insert the kings? in head
kings1 <- read.csv("data/danish_monarchs_clean.csv")
head(kings1)
kings2 <- read.csv("data/danish_monarchs_clean.csv")
head(kings2)
kings3 <- read.csv2("data/danish_monarchs_clean.csv")
head(kings3)
kings4 <- read.csv2("data/danish_monarchs_clean.csv")
head(kings4)
```
Answer: 
...
```{r}
#Step 1 identify the type of an object
class(kings1)
class(kings2)
class(kings3)
class(kings4)
...
```{r eval = TRUE}
# Step 1 COMPLETE THE BLANKS BELOW WITH YOUR CODE, then turn the 'eval' flag in this chunk to TRUE.
...
```{r}
#Step 1 remove the missing values
na.omit(kings4)
...
```{r}
# Step 1 We already have a column that shows the duration of regin called 'Periode', But we will show below how we used the mutate function to create a column called 'duration' in the kings dataset
...
```{r}
# Step 1 with the use of ChatGbt we now get
average_duration <- mean(kingsduration$Periode, na.rm = TRUE)

result <- kingsduration %>%
  filter(Periode >= average_duration)

# Step 2 now we count and print 
print(result)
count(result)
```
...
```{r}
# Step 1 With the use of ChatGbt we now get the three longest ruling monarchs
top_3_kings <- kingsduration %>%
  arrange(desc(duration)) %>%
  slice(1:3)

# Step 2 now we find the number of days ruled
